...
class TemplateApp {
    ...
    loadTemplates() {
        this.loadAllTemplates().catch(error => {
            console.error('шибка при загрузке шаблонов:', error);
            this.showNotification('шибка при загрузке шаблонов', 'error');
        });
    }
    
    loadAllTemplates() {
        return TemplateDB.getAllTemplates().then(templates => {
    ...
    expandBtn.title = 'Показать полностью';
        ...
        function linkify(text) {
            // If the text already contains <mark> highlights generated by the app,
            // we must preserve those tags and avoid double-escaping them.
            var hasMark = String(text).indexOf('<mark') !== -1;

            // Regex to find URLs
            const urlRegex = /((https?:\/\/)[\w\-._~:\/\?#\[\]@!$&'()*+,;=%]+(?![^<]*>))/gi;

            if (hasMark) {
                // Escape everything first to avoid XSS, then restore the specific
                // <mark class="search-hit"> and </mark> sequences inserted by
                // our highlighter so they render as HTML.
                var escaped = escapeHtml(text);
                escaped = escaped.replace(/&lt;mark class=&quot;search-hit&quot;&gt;/g, '<mark class="search-hit">')
                                 .replace(/&lt;\/mark&gt;/g, '</mark>');

                // Optionally keep links as plain text to avoid complex replacements
                // inside mark regions; convert newlines to <br> and return.
                return escaped.replace(/\n/g, '<br>');
            }

            // Default behaviour: escape and convert URLs to links
